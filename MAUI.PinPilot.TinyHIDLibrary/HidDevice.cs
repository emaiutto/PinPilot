 namespace MAUI.PinPilot.TinyHIDLibrary
{
    ...
    public class HidDevice : IHidDevice, IDisposable
    {
        ...
        public nint ReadHandle { get; private set; }
        public nint WriteHandle { get; private set; }
        ...
        private ReadStatus Read()
        {
            ...
            if (_deviceReadMode == DeviceMode.Overlapped)
            {
                ...
                security.lpSecurityDescriptor = nint.Zero;
                ...
                try
                {
                    unsafe
                    {
                        ...
                        {
                            var success = NativeMethods.ReadFile(ReadHandle, (nint)bufferPtr, (uint)InputBuffer.Length, out bytesRead, ref overlapped);
            ...
            else
            {
                try
                {
                    ...
                    unsafe
                    {
                        fixed (byte* bufferPtr = InputBuffer)
                        {
                            NativeMethods.ReadFile(ReadHandle, (nint)bufferPtr, (uint)InputBuffer.Length, out bytesRead, ref overlapped);
                        }
                    }
        ...
        private static HidDeviceAttributes GetDeviceAttributes(nint hidHandle)
        {
        ...
        private static HidDeviceCapabilities GetDeviceCapabilities(nint hidHandle)
        {
            var capabilities = default(NativeMethods.HIDP_CAPS);

            var preparsedDataPointer = default(nint);
        ...
        public void OpenDevice(DeviceMode readMode, DeviceMode writeMode, ShareMode shareMode)
        {
            ...
            IsOpen = ReadHandle != nint.Zero && WriteHandle != nint.Zero;

        }
         

        private static nint OpenDeviceIO(string devicePath, DeviceMode deviceMode, uint deviceAccess, ShareMode shareMode)
        {
            var security = new NativeMethods.SECURITY_ATTRIBUTES();
            var flags = 0;
            ...
            return NativeMethods.CreateFile(devicePath, deviceAccess, (int)shareMode, ref security, NativeMethods.OPEN_EXISTING, flags, hTemplateFile: nint.Zero);
        }
        ...
        private static void CloseDeviceIO(nint handle)
        {

            NativeMethods.CancelIoEx(handle, nint.Zero);
