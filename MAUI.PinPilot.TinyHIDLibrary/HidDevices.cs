namespace MAUI.PinPilot.TinyHIDLibrary
{
    public class HidDevices
    {
        ...
        public static bool IsConnected(string devicePath) => EnumerateDevices().Any(x => x.Path == devicePath);

        //public static HidDevice GetDevice(string devicePath) => Enumerate(devicePath).FirstOrDefault();

        public static HidDevice GetDevice(int VendorId, int ProductId) 
            => Enumerate(VendorId, ProductId).First();


        //public static IEnumerable<HidDevice> Enumerate() => EnumerateDevices().Select(x => new HidDevice(x.Path, x.Description));

        //public static IEnumerable<HidDevice> Enumerate(string devicePath) 
        //    => EnumerateDevices().Where(x => x.Path == devicePath).Select(x => new HidDevice(x.Path, x.Description));
        ...
        internal static IEnumerable<DeviceInfo> EnumerateDevices()
        {
            ...
            var deviceInfoSet = NativeMethods.SetupDiGetClassDevs(ref hidClass, string.Empty, hwndParent: nint.Zero, NativeMethods.DIGCF_PRESENT | NativeMethods.DIGCF_DEVICEINTERFACE);
        ...
        private static NativeMethods.SP_DEVINFO_DATA CreateDeviceInfoData()
        {
            ...
            deviceInfoData.Reserved = nint.Zero;

            return deviceInfoData;
        }

        private static string? GetDevicePath(nint deviceInfoSet, NativeMethods.SP_DEVICE_INTERFACE_DATA deviceInterfaceData)
        {
            var bufferSize = 0;
            var interfaceDetail = new NativeMethods.SP_DEVICE_INTERFACE_DETAIL_DATA { Size = nint.Size == 4 ? 4 + Marshal.SystemDefaultCharSize : 8 };

            NativeMethods.SetupDiGetDeviceInterfaceDetail(deviceInfoSet, ref deviceInterfaceData, nint.Zero, 0, ref bufferSize, nint.Zero);

            return NativeMethods.SetupDiGetDeviceInterfaceDetail(deviceInfoSet, ref deviceInterfaceData, ref interfaceDetail, bufferSize, ref bufferSize, nint.Zero) ? 
        ...
        private static string GetDeviceDescription(nint deviceInfoSet, ref NativeMethods.SP_DEVINFO_DATA devinfoData)
        {
            unsafe
            {
        ...
        private static string GetBusReportedDeviceDescription(nint deviceInfoSet, ref NativeMethods.SP_DEVINFO_DATA devinfoData)
        {
            unsafe
            {
                const int charCount = 1024;
